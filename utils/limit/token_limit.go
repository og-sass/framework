...
func NewTokenLimiter(rate, burst int, store redis.UniversalClient, key string) *TokenLimiter {
...
func (lim *TokenLimiter) reserveN(ctx context.Context, now time.Time, n int) bool {
	...
	resp, err := tokenScript.Run(ctx, lim.store,
		[]string{
			lim.tokenKey,
			lim.timestampKey,
		},
...
func (lim *TokenLimiter) waitForRedis() {
	...
	for range ticker.C {
		...
		_, err := lim.store.Ping(ctx).Result()
		cancel()
